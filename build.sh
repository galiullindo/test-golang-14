...
gcc -shared -fPIC -O2 -o bin/libcalculator.so libs/c_lib/calculator.c
echo "  -> bin/libcalculator.so"

echo "== Building Rust library =="
(cd libs/rust_lib && cargo build --release)
cp libs/rust_lib/target/release/libcalculator_rust.so bin/libcalculator_rust.so
